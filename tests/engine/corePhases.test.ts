...
describe('corePhases', () => {
  it('policy delay applies on next turn and writes timeline', () => {
    const engine = new GameEngine(20260302, 'baseline');
    const originalTax = engine.government.taxRate;
    const nextTax = 0.2;

    engine.setTaxRate(nextTax);
    expect(engine.pendingPolicies.length).toBe(1);
    expect(engine.government.taxRate).toBe(originalTax);
    expect(engine.policyTimeline.length > 0).toBe(true);
    expect(engine.policyTimeline[0].status).toBe('pending');

    engine.advanceTurn();

    expect(engine.pendingPolicies.length).toBe(0);
    expect(Math.abs(engine.government.taxRate - nextTax) < 1e-9).toBe(true);
    const taxTimeline = engine.policyTimeline.find(entry => entry.type === 'tax');
    expect(Boolean(taxTimeline)).toBe(true);
    expect(taxTimeline?.status).toBe('applied');
    expect(taxTimeline?.resolvedTurn).toBe(1);
  });

  it('demography phase marks old-age death deterministically', () => {
    ...
    expect(target.causeOfDeath).toBe('age');
  });

  it('mass emigration is capped per turn to avoid instant collapse', () => {
    ...
    expect(leftThisTurn <= cap).toBe(true);
  });

  it('random event trigger is deterministic when probabilities are controlled', () => {
    ...
    try {
      ...
      expect(activeIds.includes('drought')).toBe(true);
    } finally {
      for (const event of RANDOM_EVENTS) {
  ...
  it('scoring favors stronger economic and social outcomes', () => {
    ...
    expect(strong.wellbeingScore).toBeGreaterThan(weak.wellbeingScore);
    expect(strong.equalityScore).toBeGreaterThan(weak.equalityScore);
  });

  it('turn snapshot includes causal replay breakdown', () => {
    const engine = new GameEngine(20260306, 'baseline');
    const snapshot = engine.advanceTurn();

    expect(typeof snapshot.causalReplay.satisfaction.net).toBe('number');
    expect(typeof snapshot.causalReplay.health.net).toBe('number');
    expect(typeof snapshot.causalReplay.departures.net).toBe('number');
    expect(typeof snapshot.causalReplay.policy.taxCollected).toBe('number');
    expect(typeof snapshot.causalReplay.policy.welfarePaid).toBe('number');
    expect(typeof snapshot.causalReplay.policy.treasuryDelta).toBe('number');
    expect(snapshot.causalReplay.satisfaction.drivers.length > 0).toBe(true);
    expect(snapshot.causalReplay.health.drivers.length > 0).toBe(true);
    expect(snapshot.causalReplay.departures.drivers.length > 0).toBe(true);
  });

  it('LES demand allocation lowers quantity demanded when relative price rises', () => {
  ...
  it('locked sectors do not create unmet-need penalties in early agriculture stage', () => {
    ...
    expect(outcome.unmetNeeds.includes('goods')).toBe(false);
    expect(outcome.unmetNeeds.includes('services')).toBe(false);
    expect(outcome.satisfactionDelta >= 0).toBe(true);
  });

  it('banking savings and positive net income can ease dissatisfaction', () => {
    ...
    expect(resident.savings).toBeGreaterThan(0);
    expect(resident.lastNetIncome).toBeGreaterThan(0);
    expect(satDelta).toBeGreaterThan(0);
  });

  it('sector output follows diminishing labor returns under Cobb-Douglas scaling', () => {
    ...
    expect(output20).toBeGreaterThan(output10);
    expect(output20).toBeLessThan(output10 * 2);
  });

  it('academic calibration mode uses gentler price adjustment than baseline mode', () => {
    ...
    expect(baselineDelta).toBeGreaterThan(academicDelta);
  });
});
